...
in mind that hotline.ua's user agreement (p. 6.4) requires the administration's
written permission for automated collection; with `MARKET_FETCH_ENABLED=0`
the site runs on the curated catalog that ships with the repository.
