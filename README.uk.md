...
| Інтеграції | Anthropic Claude API (tool use), Telegram Bot API, Stripe Checkout + вебхуки, API НБУ (курс USD→UAH), ринкові дані зі сторінок товарів (за замовчуванням вимкнено, на живому демо увімкнено; див. «Живі ціни й фото») |
...
- **Запуск.** Токен від @BotFather кладеться в `backend/.env` (`TELEGRAM_BOT_TOKEN=...`; файл у `.gitignore`, змінні середовища мають пріоритет). Бот працює через long polling, публічна адреса не потрібна: `python manage.py run_telegram_bot` локально або сервіс `bot` у Docker Compose. На хостингу без фонових процесів (безкоштовний Render) замість цього вмикається вебхук `TELEGRAM_WEBHOOK=1`: Telegram сам надсилає повідомлення на `/api/telegram/webhook/`, запит перевіряється секретом, а повторна доставка того самого оновлення ігнорується. Без токена функція просто вимкнена.
...
Розгортання на VPS з автоматичним HTTPS (Caddy): **[docs/DEPLOY.md](docs/DEPLOY.md)** (англійською). Безкоштовне демо на Render + Neon: `render.yaml` і розділ «Free hosting» там само (без фонових воркерів; Telegram-бот працює через вебхук).
